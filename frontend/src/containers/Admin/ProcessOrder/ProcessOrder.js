...
import { getOrder, updateOrder, clearErrors } from '../../../actions/orderActions';
...
const ProcessOrder = ({ match, history }) => {
    ...
    const { error: updateError, success } = useSelector((state) => state.order);
    const { loading: orderLoading, error: orderError, order } = useSelector((state) => state.orderDetails);
    ...
    useEffect(() => {
        ...
        if (updateError || orderError) {
            toastError(orderError ? orderError : updateError);
    ...
    }, [dispatch, history, orderError, orderId, success, updateError]);
